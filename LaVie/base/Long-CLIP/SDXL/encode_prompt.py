...
bigG_model, _, bigG_preprocess = open_clip.create_model_and_transforms(
    'ViT-bigG-14', 
    pretrained='openclip-bigG/open_clip_pytorch_model.bin'
)
...
vitl_model, vitl_preprocess = longclip.load("longclip-L/longclip-L.pt", device=device)
vitl_model.eval()
...
with torch.no_grad():
    def encode_prompt(
        ...
        print("pipe.config.force_zeros_for_empty_prompt: ", pipe.config.force_zeros_for_empty_prompt)
