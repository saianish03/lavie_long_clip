...
from transformers import CLIPTokenizer, CLIPTextModel, CLIPTextModelWithProjection, CLIPConfig, CLIPProcessor, CLIPModel
...
def main(args):
	...
	# clip_model = CLIPModel.from_pretrained(model_id, torch_dtype=torch.float16, config=config).to(device, dtype=torch.float16)
	# clip_processor = CLIPProcessor.from_pretrained(model_id, padding="max_length", max_length=248)
	# tokenizer_one = CLIPTokenizer.from_pretrained(model_id, config=config, dtype=torch.float16)
	# text_encoder_one = clip_model.text_model
